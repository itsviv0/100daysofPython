...
def check_resources(coffee_choice):
	def check_ingredient(ingredient, required_amount):
		if required_amount > resources[ingredient]:
			print(f"There isn't enough {ingredient} for a {coffee_choice}.")
			return False
		return True

	ingredients = MENU[coffee_choice]['ingredients']
	enough =  (check_ingredient(ingredient, amount) for ingredient, amount in ingredients.items())
	return enough
